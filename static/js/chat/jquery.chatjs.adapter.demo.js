...
var DemoAdapterConstants = (function () {
    ...
    DemoAdapterConstants.LAWYER_USER_ID = 3;
...
function lookupReply(query) {
    query = query.toLowerCase();
    if (contains(query, "problem")) {
        return "The problem arises when the opposing party revises the contract without you knowing.";
    } else if (contains(query, "sign")) {
        return "You should ask for the clause on revisions to be removed before signing.";
    } else {
...
var DemoServerAdapter = (function () {
    function DemoServerAdapter(clientAdapter) {
        ...
        lawyerUser.Id = DemoAdapterConstants.LAWYER_USER_ID;
        ...
        setTimeout(function () {
            _this.getUserInfo(otherUserId, function (echobotUserInfo) {
                ...
                setTimeout(function () {
                    ...
                    echoMessage.UserFromId = echobotUserInfo.Id; //DemoAdapterConstants.ECHOBOT_USER_ID; // It will be from Echobot
                    echoMessage.RoomId = roomId;
                    echoMessage.ConversationId = conversationId;
                    echoMessage.Message = "You said: " + messageText;

                    if (echoMessage.UserFromId == DemoAdapterConstants.LAWYER_USER_ID) {
                        echoMessage.Message = lookupReply(messageText);
                    }
